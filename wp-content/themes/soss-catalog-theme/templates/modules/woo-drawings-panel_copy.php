...
/**
 * This section forces our tabs to display instead of default tabs
 */
...
if (in_array("drawings", $which_tabs)) {
  ...
  if ( have_rows( 'add_select_drawings' ) ) :
  ...
  <div class="drawing-grid">
    ...
    <?php while ( have_rows( 'add_select_drawings' ) ) : the_row(); ?>
      ...
      <?php
    // vars
        $drawing = get_sub_field( 'drawing' ); // dxf file (Array)
        $drawing_slug = get_sub_field( 'drawing_slug' ); // now to get drawings by path (Text)
        $pdf = get_sub_field( 'pdf_type' ); // pdf file (Array)
      ...
      <?php if ( $file_type == 'dxf_file' ) { // Selection is drawing file

        // vars
        $closing_comment = get_field('closing_comment', 'option');
        $standard_message = get_field( 'standard_message', 'option');

        // Two Ternary assignments ( we would get these if we are adding the drawings as a file upload.
        // Currently we are only adding drawings via FTP. We're just using a file name reference.
    ...
    <div class="modal fade" id="Modal<?php echo $drawing['id']; ?>" tabindex="-1"   role="dialog" aria-labelledby="drawingModalLabel" aria-hidden="true">
      <div class="modal-dialog modal-large" role="document">
        ...
        <?php } elseif ( $file_type == 'dxf_alt' ) { // Selection is alternate drawing
    ...
    ?>
    <div class="row">
      <div class="col-sm-8 file-listing"><button title="<?php echo 'View ' . $drawing_title . '--' . $drawing_caption; ?>" class="linkified" data-toggle="modal" data-target="#Modal<?php echo $drawing_slug; ?>"><?php echo $drawing_title; ?></button>
        &nbsp;<span class="<?php echo $type; ?>"><?php echo $extension; ?></span>
        <span class="filesize">&nbsp;(<?php echo $fileSize; ?>)</span>
      </div>
